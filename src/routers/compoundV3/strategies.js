...
/**
 * @swagger
 * /compound/v3/strategies/dfs-automation/eoa:
 *   post:
 *     summary: Subscribe to a Compound V3 Automation strategy (EOA)
 *     tags:
 *      - CompoundV3
 *     description: Subscribes to Compound V3 Automation strategy for EOA positions
 *     requestBody:
 ...
 *                   type: string
 */
router.post("/dfs-automation/eoa",
    body(["vnetUrl", "eoa", "debtSymbol", "minRatio", "maxRatio", "targetRepayRatio", "targetBoostRatio", "boostEnabled"]).notEmpty(),
    ...
    async (req, res) => {
        ...
        try {
            ...
            return res.status(500).send({ error: `Failed to subscribe to Compound V3 automation strategy (EOA) with error : ${err.toString()}` });
        }
    });

/**
 * @swagger
 * /compound/v3/strategies/dfs-automation/smart-wallet:
 *   post:
 *     summary: Subscribe to a Compound V3 Automation strategy (Smart Wallet)
 *     tags:
 *      - CompoundV3
 *     description: Subscribes to Compound V3 Automation strategy for Smart Wallet positions
 *     requestBody:
 *       description: Request body for the API endpoint
 *       required: true
 *       content:
...
router.post("/dfs-automation/smart-wallet",
    ...
    async (req, res) => {
        ...
        try {
            ...
            return res.status(500).send({ error: `Failed to subscribe to Compound V3 automation strategy (Smart Wallet) with error : ${err.toString()}` });
        }
    });
...
/**
 * @swagger
 * /compound/v3/strategies/leverage-management-on-price/smart-wallet:
 *   post:
 *     summary: Subscribe to a Compound V3 leverage management on price strategy (Smart Wallet)
 *     tags:
 *      - CompoundV3
 *     description: Subscribes to Compound V3 leverage management on price strategy for Smart Wallet positions
 *     requestBody:
 *       description: Request body for the API endpoint
 *       required: true
 *       content:
 ...
 *                   type: string
 */
router.post("/leverage-management-on-price/smart-wallet",
    body(["vnetUrl", "debtSymbol", "collSymbol", "targetRatio", "price", "priceState", "ratioState", "eoa"]).notEmpty(),
    body(["targetRatio", "price"]).isFloat({ gt: 0 }),
    body(["priceState", "ratioState"]).isIn(["under", "over"]),
    async (req, res) => {
        ...
        try {
            ...
            return res.status(500).send({ error: `Failed to subscribe to Compound V3 leverage management on price strategy (Smart Wallet) with error : ${err.toString()}` });
        }
    });

/**
 * @swagger
 * /compound/v3/strategies/close-on-price/eoa:
 *   post:
 *     summary: Subscribe to a Compound V3 close on price strategy (EOA)
 *     tags:
 *      - CompoundV3
 *     description: Subscribes to Compound V3 close on price strategy for EOA positions
 *     requestBody:
 *       description: Request body for the API endpoint
 *       required: true
 *       content:
 ...
 *                   type: string
 */
router.post("/close-on-price/eoa",
    body(["vnetUrl", "debtSymbol", "collSymbol", "stopLossPrice", "takeProfitPrice", "closeStrategyType", "eoa"]).notEmpty(),
    ...
    async (req, res) => {
        ...
        try {
            ...
            return res.status(500).send({ error: `Failed to subscribe to Compound V3 close on price strategy (EOA) with error : ${err.toString()}` });
        }
    });

/**
 * @swagger
 * /compound/v3/strategies/close-on-price/smart-wallet:
 *   post:
 *     summary: Subscribe to a Compound V3 close on price strategy (Smart Wallet)
 *     tags:
 *      - CompoundV3
 *     description: Subscribes to Compound V3 close on price strategy for Smart Wallet positions
 *     requestBody:
 *       description: Request body for the API endpoint
 *       required: true
 *       content:
 ...
 *                   type: string
 */
router.post("/close-on-price/smart-wallet",
    body(["vnetUrl", "debtSymbol", "collSymbol", "stopLossPrice", "takeProfitPrice", "closeStrategyType", "eoa"]).notEmpty(),
    body(["stopLossPrice", "takeProfitPrice"]).isFloat({ min: 0 }),
    body("closeStrategyType").isInt({ min: 0, max: 7 }),
    async (req, res) => {
        ...
        try {
            ...
            return res.status(500).send({ error: `Failed to subscribe to Compound V3 close on price strategy (Smart Wallet) with error : ${err.toString()}` });
        }
    });
